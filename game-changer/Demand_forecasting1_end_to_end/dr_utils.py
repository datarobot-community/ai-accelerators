...
def create_dataset_from_data_source(data_source_name, query, data_store_id, credential_id):
    ...
    if len(data_sources) > 0:
        ...
        data_source = dr.DataSource.create(data_source_type='jdbc', canonical_name=data_source_name, params=ds_params)
        print('new data source:', data_source)

    dataset = dr.Dataset.create_from_data_source(data_source_id=data_source.id, credential_id=credential_id, max_wait=max_wait, do_snapshot=True)
...
def make_series_stats(data, date_col, series_id, target, freq=1):
    ...
    data_mode = data_tmp.groupby(series_id)[['gap']].agg(gap_mode=('gap', pd.Series.mode)).reset_index()
    data_tmp = data_tmp.merge(data_mode, how='left', on=series_id)
    data_tmp['gap_uncommon'] = ((data_tmp['gap'] != data_tmp['gap_mode']) & (data_tmp['gap']).notna() & (data_tmp['gap_mode']).notna()) * 1
    ...
    data_agg['duration'] = ((data_agg['date_col_max'] - data_agg['date_col_min']).dt.days) // freq + 1
...
def identify_spikes(data, date_col, series_id, target, span=5, threshold=4):
    ...
    tmp['EWA'] = tmp.groupby(series_id)[target].transform(lambda x: x.ewm(span=span).mean()).fillna(value=1)
    tmp['SD']  = tmp.groupby(series_id)[target].transform(lambda x: x.ewm(span=span).std()).fillna(value=1)
...
def save_project_meta(file_to_save, results):
    ...
    cols = ['project', 'project_name', 'project_type', 'cluster_id', 'cluster_name', 'description']
...
def run_project(data, params, notes=''):
    ...
    feature_settings = [dr.FeatureSettings(c, known_in_advance=True) for c in proj_params.pop('features_known_in_advance', [])]
    feature_settings += [dr.FeatureSettings(c, do_not_derive=True) for c in proj_params.pop('do_not_derive_features', [])]
...
def get_leaderboard(proj, metrics = [], rename_proj_metric=False):
    ...
    model_scores = model_scores.sort_values([f'{metric}_backtests', f'{metric}_backtest1'], 
                                                          ascending=ascending, na_position='last')
    model_scores.reset_index(drop=True, inplace=True)
    if rename_proj_metric:
        model_scores.rename(columns={f'{metric}_backtests': 'metric_backtests', f'{metric}_backtest1': 'metric_backtest1'}, 
                            inplace=True)
    return model_scores

def make_deployments(projects_dct, file_to_save=None, feature_drift_enabled=False):
    """
    Args:
    ...
    for cl, vals in deployments_dct.items():
        ...
        deployment = make_deployment(project, name=project.project_name, model_id=model_id,
    ...
    if file_to_save is not None:
        ...
        cols = ['project', 'project_name', 'project_type', 'cluster_id', 'cluster_name', 
                'description', 'deployment_id', 'deployment']
        df_log = df_log[cols].copy()
        df_log.to_csv(file_to_save, header=not os.path.isfile(file_to_save), mode='a')

    return deployments_dct

def make_deployment(project, name=None, model_id=None, feature_drift_enabled=False):
    """deploys recommended or provided model

    Args:
        project (dr.Project):
        name (str, optional): a name for the deployment. Defaults to None.
        model_id (str, optional): model_id to deploy, defaults to None, which selects the Recommended model from the project.
    ...
    deployment = dr.Deployment.create_from_learning_model(model_id=model_id,
        label=name, default_prediction_server_id=pred_serv_id)

    print(f"Deployment ID: {deployment.id}; URL: https://app.datarobot.com/deployments/{deployment.id}/overview")
    print()

    # try:
    #     deployment.update_prediction_intervals_settings(percentiles=[90],
    #         enabled=True, max_wait=3600)
    # except Exception as e:
    #     print(str(e))

    if feature_drift_enabled:
        try:
            deployment.update_drift_tracking_settings(feature_drift_enabled=True, max_wait=3600)
        except Exception as e:
            print(str(e))
...
def make_predictions(deployments, intake_settings, output_settings, wait_for_completion=True, forecast_point=None,
                     passthrough_columns_set='all', passthrough_columns=None):
    jobs = []
    ...
    for _, vals in deployments.items():
        ...
        if vals['project_type'] in ('all series', 'segmented'):
            jobs.append(make_predictions_from_deployment(deployment, intake_settings, output_settings, wait_for_completion,
                                                         forecast_point, passthrough_columns_set, passthrough_columns))
        else:
            # todo
            pass
...
def make_predictions_from_deployment(deployment, intake_settings, output_settings, wait_for_completion=True, forecast_point=None,
                                     passthrough_columns_set='all', passthrough_columns=None):
...
def create_preds_job_definitions(deployments, intake_settings, output_settings, enabled=False, schedule=None,
                                 passthrough_columns_set='all', passthrough_columns=None):

    for pid, vals in deployments.items():
        print(str(dt.now()), 'start:', vals['project_name'], 'create predictions job definition')
        deployment = vals['deployment']

        if vals['project_type'] in ('all series', 'segmented'):
            preds_job_def = create_preds_job_definition(deployment, intake_settings, output_settings, enabled=enabled, schedule=schedule,
                                                        passthrough_columns_set=passthrough_columns_set, passthrough_columns=passthrough_columns)
...
def create_preds_job_definition(deployment, intake_settings, output_settings, enabled=False, schedule=None,
                                passthrough_columns_set='all', passthrough_columns=None):
    ...
    preds_job_def = dr.BatchPredictionJobDefinition.create(enabled=enabled,
        ...
        try:
            ...
            for train_pred in train_preds:
                if train_pred.model_id == model.id and train_pred.data_subset == data_subset:
                    preds_tmp = dr.TrainingPredictions.get(project.id, 
                                    train_pred.prediction_id).get_all_as_dataframe()
    ...
    for _, vals in projects_dct.items():
        ...
        if project.is_segmented:
            ...
            for row in df_segm.itertuples():
                ...
                print(str(dt.now()), 'start:', vals['project_name'], f'training predictions: {cluster_name}')
                ...
                preds_tmp.rename(columns={
                    'timestamp': date_col, 
                    'series_id': series_id, 
                    'prediction': f'{target}_prediction'}, inplace=True)
...
def smape(data, target, pred_col):
    return 100.0 * np.mean(np.abs(data[pred_col] - data[target]) / ((np.abs(data[pred_col]) + np.abs(data[target])) / 2))
...
def plot_series_count_over_time(data, date_col, series_id):
    ax = data.groupby(date_col)[series_id].nunique().plot(figsize=(15,7), color=[dr_orange])
...
def plot_accuracy_over_time(project, model, backtest=0, forecast_distance=None,
                            series_id=None, max_wait=3600):
    project = project.set_worker_count(-1)
    try:
    ...
    model_aot = model.get_accuracy_over_time_plot(
        backtest=backtest, forecast_distance=forecast_distance,
        series_id=series_id, max_wait=max_wait) 
    data = pd.DataFrame(model_aot.bins)

    ax = data.plot(x='start_date', y=['actual', 'predicted'], ylim=0, figsize=(15,7),
                   color=[dr_orange, dr_blue], marker='o', markersize=10)
    ax.set_facecolor(dr_dark_blue)
    ax.xaxis.grid(True, alpha=0.2)
    ax.yaxis.grid(False)
...
def plot_stability_scores(project, model, metric=None):
    ...
    ax = scores.plot(figsize=(15,7), ylim=0, color=[dr_orange],
...
def plot_predictions(data, test_start_date, date_col, target, freq):
    ...
    ax = data_agg.plot(figsize=(15,7), ylim=0, rot=45, color=[dr_orange, dr_blue], marker='o', markersize=10)
    ax.set_facecolor(dr_dark_blue)
...
def plot_drift_data(data, drift_score_threshold=0.2, feature_impact_threshold=0.5):    
    data_tmp = data.copy()
    data_tmp['color'] = np.where(data_tmp['drift_score'] >= drift_score_threshold,
                                 np.where(data_tmp['feature_impact'] >= feature_impact_threshold,
                                          dr_red, dr_orange), 
                                 dr_green)
    ...
    data_tmp['drift_score'] = np.where(data_tmp['drift_score_orig'] <= 1,
                                       data_tmp['drift_score_orig'],
                                       data_tmp['drift_score_rank'] + 1)
    
    ax = data_tmp.plot(kind='scatter', x='feature_impact', y='drift_score', s=70, marker='o',
                      color=data_tmp['color'], figsize=(15, 7))
    ...
    for row in data_tmp[(data_tmp['color'].isin([dr_red])) | (data_tmp['feature_impact'] == 1.0)].itertuples():
        plt.annotate(row.feature, (row.feature_impact-0.1, row.drift_score + 0.02), color='white')
    ax.set_facecolor(dr_dark_blue)
    ax.xaxis.grid(False)
    ax.yaxis.grid(False)
