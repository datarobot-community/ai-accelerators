...
def extract_d2_diagrams(markdown_file):
    ...
    for i, diagram_code in enumerate(matches):
        ...
        diagrams.append(
            {"title": title, "filename": filename, "code": diagram_code.strip()}
        )
...
def check_d2_installed():
    ...
    try:
        result = subprocess.run(
            ["d2", "--version"], capture_output=True, text=True, timeout=5
        )
...
if __name__ == "__main__":
    ...
    if not check_d2_installed():
        ...
        print(
            "  Linux:   curl -fsSL https://d2lang.com/install.sh | sh -s -- --prefix ~/.local"
        )
