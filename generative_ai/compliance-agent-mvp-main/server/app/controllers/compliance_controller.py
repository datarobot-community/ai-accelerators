...
def get_confidence_threshold() -> int:
    ...
    try:
        ...
        if threshold < 0 or threshold > 100:
            print(
                "Warning: GATEKEEPER_CONFIDENCE_THRESHOLD out of range (0-100), using default 70",
                file=sys.stderr,
            )
            return 70
        return threshold
        ...
        print(
            "Warning: Invalid GATEKEEPER_CONFIDENCE_THRESHOLD value, using default 70",
            file=sys.stderr,
        )
        return 70
...
print(
    f"Info: Gatekeeper confidence threshold set to {CONFIDENCE_THRESHOLD}",
    file=sys.stderr,
)
...
async def get_regulations():
    ...
    for regulation_name, _ in regulations:
        ...
        result.append({"filename": regulation_name, "displayName": display_name})
...
async def upload_files(
    ...
    async def process():
        ...
        user_uploaded_regulations = (
            []
        )  # Store user-uploaded policy files as (filename, markdown_content) tuples
        ...
        if policy_files:
            for policy_file in policy_files:
                ...
                file_id = hashlib.md5(
                    f"{policy_file.filename}_{timestamp}".encode()
                ).hexdigest()
                policy_file_path = os.path.join(
                    TMP_DIR, f"policy_upload_{file_id}_{safe_filename}"
                )
                ...
                policy_name = policy_custom_names_dict.get(
                    policy_file.filename, policy_file.filename
                )
                ...
                try:
                    ...
                    try:
                        ...
                        markdown_content = await loop.run_in_executor(
                            None, file_to_markdown, policy_file_path
                        )
                        # Store as (custom_name, markdown_content) tuple
                        user_uploaded_regulations.append(
                            (policy_name, markdown_content)
                        )
                    except Exception as e:
                        print(
                            f"Error converting policy file {policy_file.filename} to markdown: {e}"
                        )
                        # Continue with other policy files even if one fails
                        if (
                            session_id in user_policy_files
                            and original_filename in user_policy_files[session_id]
                        ):
                            del user_policy_files[session_id][original_filename]
                        if (
                            session_id in policy_custom_name_map
                            and policy_name in policy_custom_name_map[session_id]
                        ):
                        ...
                        if os.path.exists(policy_file_path):
                            try:
                                ...
                                print(
                                    f"Error removing policy file {policy_file_path}: {e2}"
                                )
                except Exception as e:
        ...
        for file in files:
            ...
            file_path = os.path.join(
                TMP_DIR, f"compliance_upload_{timestamp}_{safe_filename}"
            )
            # Validate path is within TMP_DIR to prevent path traversal attacks
            ...
            try:
                ...
                with open(file_path, "wb") as f:
                    ...
                    for i in range(0, len(file_bytes), chunk_size):
                        ...
                        progress = (
                            int((bytes_written / total_size) * 100)
                            if total_size > 0
                            else 100
                        )
                        await emitter.emit(
                ...
                # Emit parsing status
                await emitter.emit(
                    {"type": "parsing", "data": {"filename": file.filename}}
                )
                ...
                await emitter.emit(
                    {"type": "validating", "data": {"filename": file.filename}}
                )
                ...
                try:
                    ...
                    validation_result = await loop.run_in_executor(
                        None, validate_document_relevance, file_content
                    )
                    ...
                    if status != "VALID" or confidence < CONFIDENCE_THRESHOLD:
                        ...
                        reason = validation_result.get(
                            "reason",
                            "Document is not relevant to telecom/domain compliance",
                        )
                        await emitter.emit(
                            {
                                "type": "document_invalid",
                                "data": {"filename": file.filename, "reason": reason},
                            }
                        )
                        # Clean up and continue to next file
                        if os.path.exists(file_path):
                ...
                await emitter.emit(
                    {"type": "file_validated", "data": {"filename": file.filename}}
                )
                ...
                await emitter.emit({"type": "error", "data": {"message": error_msg}})
                # Clean up on error
                if os.path.exists(file_path):
                    try:
                        os.remove(file_path)
        ...
        for valid_file in valid_files:
            try:
                ...
                async def on_verification_progress(
                    regulation_name: str, regulation_index: int, total_regulations: int
                ):
                    # Just log regular info without using sys.stderr to avoid mixing with error logs
                    print(
                        f"Info: {regulation_index}/{total_regulations} - Verifying against {regulation_name}"
                    )
                    await emitter.emit(
                        {
                            "type": "verifying",
                ...
                async def on_regulation_complete(
                    regulation_name: str, regulation_index: int, total_regulations: int
                ):
                    print(
                        f"Info: {regulation_index}/{total_regulations} - Completed {regulation_name}"
                    )
                    await emitter.emit(
                        {
                ...
                issues = await service.verify_against_regulations(
                    ...
                    user_uploaded_regulations=(
                        user_uploaded_regulations if user_uploaded_regulations else None
                    ),
        ...
        await emitter.emit({"type": "complete", "data": {"issues": all_issues}})
        ...
        async def cleanup_session_files():
            ...
            if session_id in user_policy_files:
                for original_filename, policy_path in list(
                    user_policy_files[session_id].items()
                ):
...
async def get_user_policy_file(session_id: str, filename: str) -> FileResponse:
    ...
    if (
        session_id in policy_custom_name_map
        and filename in policy_custom_name_map[session_id]
    ):
