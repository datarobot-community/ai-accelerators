...
def convert_docx_to_markdown(input_path: Path) -> str:
    ...
    if mammoth is None:
        raise RuntimeError(
            "mammoth is required. Please install dependencies from requirements.txt"
        )
    if html_to_md is None:
        raise RuntimeError(
            "markdownify is required. Please install dependencies from requirements.txt"
        )
...
def file_to_markdown(
    file_path: str | Path, output_path: Optional[str | Path] = None
) -> str:
    """
