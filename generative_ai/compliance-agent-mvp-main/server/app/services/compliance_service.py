...
class ComplianceService:
    ...
    async def _verify_single_regulation(
        ...
        issues_data = await loop.run_in_executor(
            None, evaluate_compliance, client, model, messages, json_schema
        )
    ...
    async def verify_against_regulations(
        ...
        if selected_regulations is not None:
            ...
            if len(selected_regulations) > 0:
                ...
                regulations = [
                    (name, content)
                    for name, content in all_regulations
        ...
        async def process_regulation_with_callbacks(
            regulation_index: int, regulation_name: str, regulation_content: str
        ) -> List[ComplianceIssue]:
            ...
            is_user_uploaded = (
                user_uploaded_regulations is not None
                and regulation_name in user_uploaded_names
                and regulation_index >= kb_regulations_count
            )
            ...
            try:
                ...
                if on_regulation_complete:
                    await on_regulation_complete(
                        display_name, regulation_index + 1, total_regulations
                    )
                ...
                print(
                    f"Error processing regulation {regulation_name}: {e}",
                    file=sys.stderr,
                )
        ...
        tasks = [
            process_regulation_with_callbacks(
                regulation_index, regulation_name, regulation_content
            )
            for regulation_index, (regulation_name, regulation_content) in enumerate(
                regulations
            )
        ]
        ...
        for result in results:
            if isinstance(result, Exception):
                ...
                print(
                    f"Error in parallel regulation processing: {result}",
                    file=sys.stderr,
                )
