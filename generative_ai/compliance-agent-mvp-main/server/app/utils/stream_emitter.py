...
import json
from typing import Any, AsyncGenerator, Awaitable, Callable, Generic, Optional, TypeVar
...
class StreamEmitter:
    ...
    async def emit_error(self, message: str, error_type: str = "error") -> None:
        await self.emit({"type": error_type, "data": {"message": message}})
    ...
    def stream_response(
        ...
        return StreamingResponse(self._generate(task_obj), media_type=self.media_type)
...
async def stream_with_emitter(
    func: Callable[[StreamEmitter], Awaitable[None]], keep_alive: bool = True
) -> StreamingResponse:
    emitter = StreamEmitter()
