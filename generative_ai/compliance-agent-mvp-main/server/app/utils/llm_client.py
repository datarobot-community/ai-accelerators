...
def create_llm_client(model_name: str = None) -> tuple[OpenAI, str]:
    ...
    if mode == "direct-llm":
        ...
        if not deployed_llm_url:
            raise RuntimeError(
                "LLM_ENDPOINT environment variable is required for direct-llm mode"
            )
        ...
        if not dr_endpoint:
            raise RuntimeError(
                "DATAROBOT_ENDPOINT environment variable is required for dr-gateway mode"
            )
        if not dr_token:
            raise RuntimeError(
                "DATAROBOT_API_TOKEN environment variable is required for dr-gateway mode"
            )
        ...
        raise RuntimeError(
            f"Invalid MODE value: {mode}. Must be 'dr-gateway' or 'direct-llm'"
        )
