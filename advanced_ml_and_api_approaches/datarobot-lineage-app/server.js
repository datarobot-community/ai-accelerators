...
const server = http.createServer((req, res) => {
	...
	if (method === "POST") {
		...
		const edgeOutputFile = path.join(__dirname, `${useCaseId}_edges.json`);
		const nodeOutputFile = path.join(__dirname, `${useCaseId}_nodes.json`);
		if (!fs.existsSync(nodeOutputFile)) {
			runPythonScript(
				path.join(__dirname, "create_graph_from_use_case.py"),
				["--use-case-id", useCaseId, "--node-output-file", nodeOutputFile, "--edge-output-file", edgeOutputFile],
				(err, result) => {
					if (err) {
						console.error(`error -> ${err}`);
			...
			console.log(`${nodeOutputFile} exists`);
			res.writeHead(200, { "Content-Type": "text/plain" });
			res.end("refresh", "utf-8");
		}
