...
cols_to_exclude = [date_col, series_id, target,
                   'FORECAST_POINT', 'FORECAST_DISTANCE', 'DEPLOYMENT_APPROVAL_STATUS',
                   'is_pred_period', 'is_updated', 'run_id', 'params']
...
if uploaded_file is not None:
    ...
    selection = st.radio("Modify known in advance (KA) values:", ('Yes', 'No'),
                        on_change=au.clear_cache, kwargs={'vals': ['preds']},
                        index=1, key='modify_ka_val_radio')
    ...
    if selection == 'Yes':
        ...
        if len(ka_cols) > 0:
            ...
            ka_cols_selected = st.multiselect('Select KA columns:', ka_cols,
                                            default=None, key='ka_msel',
                                            on_change=au.clear_cache, kwargs={'vals': ['preds']})
            ...
            with st.container():
                ...
                for i, ka_col in enumerate(ka_cols_selected):
                    ...
                    ka_col_val = st.selectbox(f'Select "{ka_col}" value:', ['<select>', '<new>'] + ka_col_vals,
                                                    on_change=au.clear_cache, kwargs={'vals': ['preds']},
                                                    index=0, key=f'ka_val_selectbox{i}')
                    if ka_col_val == '<new>':
                ...
                if len(ka_cols_dct) == len(ka_cols_selected):
                    dates_sel = st.multiselect('Select dates to change the value:', ['all'] + dates_lst,
                                                       default='all', key='dates_msel',
                                                       on_change=au.clear_cache, kwargs={'vals': ['preds']},)
                    if len(dates_sel) == 0:
                        st.stop()
                    ...
                    preds = au.add_make_predictions_button(df_to_preds, deployment, date_col, series_id, target,
                                                            run_id, params=run_params)
            ...
            preds = au.add_make_predictions_button(df_to_preds, deployment, date_col, series_id, target, 'Uploaded data')
            if preds is not None:
                st.session_state.preds = preds.copy()
    ...
    if 'preds_hist' in st.session_state:
        ...
        id_params = {k: ';'.join(st.session_state.params_hist.get(k, ['Uploaded data'])) for k in st.session_state.preds_hist.keys()}
        ...
        down_name = st.sidebar.selectbox(f'Select predictions to download:', param_names,
                                    index=0, key='run_down_selectbox')
        
        if down_name == '<select>':
            down_preds = None
            preds_cols = None
        elif down_name == 'All predictions':
            down_preds, preds_cols = au.combine_predictions(st.session_state.preds_hist, date_col, series_id, target)
        ...
        with col1:
            param_name = st.selectbox(f'Select predictions:', param_names,
                                            index=0, key='run_param_selectbox')

            if param_name == '<select>':
                st.stop()
            elif param_name == 'All predictions':
                preds, cols_preds = au.combine_predictions(st.session_state.preds_hist,
                                    date_col, series_id, target)
            ...
            if param_name not in ('<select>', 'All predictions'):
                pred_int_cols = [c for c in preds.columns if c.startswith('prediction_') and '_percentile_' in c]
            ...
            agg_granularity = st.selectbox(f'Select aggregation granularity:', agg_granularities,
                                        index=0, key='ragg_param_selectbox')
            ...
            if agg_granularity == '<select>':
                ...
                plot_agg_func = st.selectbox('Aggregation:', ['<select>', 'sum', 'mean'],
                                        index=1, key='plot_agg_func_selectbox')
                if plot_agg_func != '<select>':
                    with col2:    
                        au.plot_predictions_agg_overall(preds, date_col, cols_to_plot, plot_agg_func)
                ...
                group_col = st.selectbox('Select a group:', ['<select>'] + group_cols,
                                                    index=0, key='group_selectbox')
                if group_col != '<select>':
                    group_vals = sorted(preds[group_col].unique())
                    group_val = st.selectbox(f'Select "{group_col}" value:', ['<select>'] + group_vals,
                                            index=0, key='group_val_selectbox')
                    if group_val != '<select>':
                        plot_agg_gr_func = st.selectbox('Aggregation:', ['<select>', 'sum', 'mean'],
                                                    index=1, key='plot_agg_func_gr_selectbox')
                        if plot_agg_gr_func != '<select>':
                            with col2:
                                au.plot_predictions_agg_group(preds, date_col, group_col, group_val, cols_to_plot,
                                                        plot_agg_gr_func)

            elif agg_granularity == 'Series':
                # st.write("Single series")
                sids = sorted(preds[series_id].unique())
                sid = st.selectbox('Select a series:', ['<select>'] + sids,
                                            index=0, key='sid_selectbox')
                if sid != '<select>':
                    with col2:
