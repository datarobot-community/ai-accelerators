...
def make_deployments(projects_dct, file_to_save=None, target_drift_enabled=False, feature_drift_enabled=False):
    """
    Args:
...
def make_deployment(project, name=None, model_id=None, target_drift_enabled=False, feature_drift_enabled=False):
    ...
    pred_serv_id = dr.PredictionServer.list()[2].id
    ...
    try:
        deployment.update_drift_tracking_settings(target_drift_enabled=target_drift_enabled,
                                                  feature_drift_enabled=feature_drift_enabled, max_wait=3600)
    except Exception as e:
        print(str(e))
...
def plot_series_count_over_time(data, date_col, series_id):
    ax = data.groupby(date_col)[series_id].nunique().plot(figsize=(15,7), color=[dr_orange], ylim=0)
