...
def compute_attribution(
    ...
    """
    Calculates incremental SHAP values to measure marketing channel contributions.

    This function computes the incremental impact of marketing spend by comparing
    model predictions with and without marketing spend, then calculating the
    difference to isolate the contribution of each marketing channel.

    Parameters:
    ----------
    deployment_id: str
        ID of the DataRobot deployment to use for predictions
    data: pd.DataFrame
        Input data containing marketing spend features and other predictors
    client: dr.Client
        DataRobot client instance for making API calls
    target_column: str
        Name of the target column in the model
    marketing_spend_features: List[str]
    ...
    pd.DataFrame
        ...
        - "Date": Original date from the input data
        - "{target_column}": Predicted values with marketing spend
        - "Baseline": Predicted values without marketing spend
        - "Marketing Effect": Overall impact of all marketing channels
        - Individual columns for each marketing channel's incremental contribution

    Notes:
    -----
    - This function assumes the existence of a separate function 'deployment_predict'
    - The function creates a counterfactual by setting all marketing spend values to zero
    - The difference between predictions with and without marketing spend represents
      the incremental contribution of each channel
    """
    # First make prediction as we normally would
    pred_w_marketing_spend = deployment_predict(
        deployment_id=deployment_id,
        data=data,
        client=client,
    )

    # Zero out marketing columns
    data_zero = data.reset_index().copy()
    data_zero[marketing_spend_features] = 0

    # Now make predictions assuming no marketing spend occurred
    pred_wo_marketing_spend = deployment_predict(
        deployment_id=deployment_id,
        data=data_zero,
        client=client,
    )
    ...
    # Subtract and rename
    pred_incremental = pred_w_marketing_spend[
        ["Prediction"] + marketing_spend_features
    ].sub(pred_wo_marketing_spend[["Prediction"] + marketing_spend_features])

    # Rename columns
    pred_w_marketing_spend = pred_w_marketing_spend.rename(
        columns={"Prediction": target_column}
    )
    pred_wo_marketing_spend = pred_wo_marketing_spend.rename(
        columns={"Prediction": "Baseline"}
    )
    pred_incremental = pred_incremental.rename(
        columns={"Prediction": "Marketing Effect"}
    )

    # Reorganize into final output
    pred_output = (
        data_zero[["Date"]]
        .join(pred_w_marketing_spend[[target_column]])
        .join(pred_wo_marketing_spend[["Baseline"]])
        .join(pred_incremental[["Marketing Effect"] + marketing_spend_features])
    )

    # Post-process predictions
    pred_output_post = post_process_incremental_values(
        shap_incremental=pred_output,
        data=data,
        marketing_spend_features=marketing_spend_features,
    )

    # For clarity, adding "Contribution" so it's clear
    pred_output_post.columns = [
        x.replace("Spend", "Spend Contribution") if x.endswith("Spend") else x
        for x in pred_output_post.columns
    ]

    return pred_output_post
